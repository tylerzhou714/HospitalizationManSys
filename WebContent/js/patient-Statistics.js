...
function showList(lists, start, end) {
	if (lists.length > 0) {
		for (var i = start; i < end; i++) {
			...
			var $tr = trStyle + "<td class='node'>" + patient.parameter_values
					+ "</td>" + "<td class='num'>" + patient.parameter_name
					+ "</td>" + "<td class='node'>" + patient.inNum + "</td>"
...
function showbar(lists) {
	...
	/*
	 * 坑2：图例legend(图例)没有option.legend.data方法， 动态加载图例要使用option.legend = 数据源，
	 * 其中数据源是data列表：data[] 具体使用方法见showpie(lists)函数的动态构造图例部分
	 */
	// 动态构造纵坐科室名称
	option.yAxis[0].data = [];
