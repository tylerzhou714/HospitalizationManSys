...
function showList(lists, start, end) {
	if (lists.length > 0) {
		for (var i = start; i < end; i++) {
			...
			var measureTime = patient.measureTime.substring(0,
					patient.measureTime.length - 2);
			var $tr = trStyle + "<td class='num'>" + patient.bedNo + "</td>"
...
function showline(lists) {
	...
	var option = {
		...
		legend : {
			left : '25%',
			data : [ '体温(℃)', '心率(次/分)', '血糖(mmol/l)', '血压-收缩压(mmHg)',
					'血压-舒张压(mmHg)' ]
		},
		grid : {
			left : '3%',
	...
	for (var i = 0; i < lists.length; i++) {
		var Time = lists[i].measureTime.substring(0,
				lists[i].measureTime.length - 2);
...
// 生成折线图的条件检测
/*
 * 折线图反应的是一个人(人数=1)的多次(≥2次)体征数据的变化趋势 返回的结果中患者ID有多个则不符合生成折线图的条件
...
function line_check(lists) {
	...
	if (lists.length > 1) {
		...
		for (var i = 1; i < lists.length; i++) {
			if (lists[i].patientId != firstpatientid) {
				counter += 1;
			}
		}
		if (counter == 0) {
